...
# ============================ CONFIG ============================
# DATA_PATH None ise script, klasorde otomatik csv/xlsx arar (zip'ten cikardigin dosya).
# Istersen tam yolu yazabilirsin: DATA_PATH = 'ai_student_impact.csv'
DATA_PATH = 'ai_student_impact_dataset.csv'

# Tahmin etmek istedigin hedef kolon. Bu veri setinde dogal secimler:
#   'Burnout_Risk_Level'  -> siniflandirma (Low/Medium/High)
#   'Post_GPA'            -> regresyon
...
print("=" * 60)
print("AI IMPACT ON STUDENTS - XGBOOST")
print("=" * 60)
...
# Tamamen bos satir/kolonlari at
df = df.dropna(how='all').dropna(axis=1, how='all')

# Hedef kolonu bos olan satirlari at (bunlar tahmin icin kullanilamaz)
before = df.shape[0]
df = df.dropna(subset=[TARGET_COLUMN])
print(f"    Hedef '{TARGET_COLUMN}' bos olan {before - df.shape[0]} satir atildi.")

# Var olan kimlik kolonlarini dusur
...
# Eksik verileri doldur: sayisal -> medyan, kategorik -> mod
...
# Gorev tipini otomatik belirle:
# hedef sayisal ve cok benzersiz degere sahipse -> regresyon, aksi halde siniflandirma
...
# Kategorik ozellikleri label-encode et (XGBoost sayisal girdi ister)
encoders = {}
...
# Siniflandirmada hedefi de encode et (etiket isimlerini sakla)
target_encoder = None
