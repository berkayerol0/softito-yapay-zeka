...
print("=" * 60)
print("INSURANCE - CLASSIFICATION (LOGISTIC REGRESSION)")
print("=" * 60)
...
df['CostClass'] = np.where(df[SOURCE_COLUMN] > median_val, 'High', 'Low')
print(f"    Medyan {SOURCE_COLUMN}: {median_val:.2f}")
print(f"    Sinif dagilimi:\n{df['CostClass'].value_counts().to_string()}")

# charges kolonunu ozelliklerden cikar (hedef ondan turetildi, sizinti olmasin)
y_raw = df['CostClass']
X = df.drop(columns=['CostClass', SOURCE_COLUMN])
...
results = pd.DataFrame(X, columns=X.columns).iloc[:0]  # kolon sirasi icin
results = X.copy()
results = results.loc[df.index]  # hizalama
out = df.copy()
out['Predicted'] = target_le.inverse_transform(model.predict(X_scaled))
out[['CostClass', 'Predicted']].to_csv('output/clf_predictions.csv', index=False)
