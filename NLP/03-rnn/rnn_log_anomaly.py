"""
HDFS Log Anomali Tespiti — Vanilla RNN (PyTorch)
===================================================
Veri seti : LogHub HDFS_v1
             Event_traces.csv  — olay dizileri + etiketler
Görev     : Her HDFS oturumunu (block) Normal/Anomali olarak sınıflandır
...
MAX_SEQ_LEN    = 50
TEST_SIZE      = 0.2
SAMPLE_FRAC    = 1.0   # hızlı test için 0.1 yap
...
if not os.path.exists(traces_path):
    raise FileNotFoundError(
        ...
        "    'data/README.md' dosyasını okuyarak\n"
        "    Event_traces.csv dosyasını 'data/' klasörüne koy.\n"
        "    Kaynak: https://github.com/logpai/loghub"
    )
...
if SAMPLE_FRAC < 1.0:
    ...
    print(f"  [Örnekleme] {len(df)} block kullanılıyor (SAMPLE_FRAC={SAMPLE_FRAC})")

# Label: Success → 0, Fail → 1
df["label_int"] = (df["Label"] == "Fail").astype(int)

print(f"  Toplam block : {len(df):,}")
print(f"  Normal       : {(df['label_int']==0).sum():,}")
print(f"  Anomali      : {(df['label_int']==1).sum():,}")

# ─────────────────────────────────────────────
# 2. Dizi Ayrıştırma ve Kodlama
# ─────────────────────────────────────────────
def parse_features(feat_str):
    """'[E5,E22,E5,...]' → ['E5', 'E22', 'E5', ...]"""
    return re.findall(r'E\d+', str(feat_str))

df["tokens"] = df["Features"].apply(parse_features)

all_events = set()
for tokens in df["tokens"]:
    all_events.update(tokens)

event2idx = {e: i + 1 for i, e in enumerate(sorted(all_events))}  # 0 = padding
VOCAB_SIZE = len(event2idx) + 1
print(f"  Benzersiz olay türü: {len(event2idx)}")
...
axes[1].set_ylabel("Block Sayısı")
axes[1].legend()
...
with plt.style.context("dark_background"):
    ...
    # Sağ — Doğruluk
    baseline = (y_test == 0).sum() / len(y_test) * 100  # çoğunluk sınıfı tahmini
