...
Tek bir Python dosyasında uçtan uca bir proje:
  1) IMDB film yorumu veri setini indirir (50.000 yorum, pozitif/negatif)
  2) Metni temizler, kelime dağarcığı kurar, dizilere çevirir
  3) BiLSTM + Bahdanau (additive) Attention modelini PyTorch'ta SIFIRDAN eğitir
     (Bahdanau, Cho & Bengio, 2014 — attention mekanizmasının ilk önerildiği makale)
  4) Modeli değerlendirir (doğruluk, F1, karmaşıklık matrisi)
  5) ASIL GÖSTERİ: attention ağırlıklarını kelime bazlı ısı haritası olarak
     görselleştirir — model kararını verirken HANGİ kelimelere baktı?
...
Bu hem daha iyi performans hem de YORUMLANABİLİRLİK (interpretability) getiriyor.

Veri seti: IMDB Movie Reviews (50.000 yorum, dengeli pozitif/negatif)
==============================================================================
"""
...
N_SAMPLES = 12000          # hız/kalite dengesi için alt küme (train 10k + test 2k)
MAX_VOCAB = 15000
MAX_LEN = 150
EMB_DIM = 100
HIDDEN_SIZE = 96
...
def download_imdb():
    if os.path.exists(IMDB_PATH):
        print(f"IMDB verisi zaten mevcut: {IMDB_PATH}"); return
    print("IMDB veri seti indiriliyor (~65MB)...")
...
class BahdanauAttention(nn.Module):
    """
    Attention'ın çekirdek fikri: LSTM'in HER zaman adımındaki çıktısını
    (klasik yaklaşımda sadece SON çıktı kullanılırdı) bir "önem skoru" ile
    ağırlıklandırıp topla. Skor, küçük bir feed-forward ağ (W, v) ile
    öğrenilir — bu yüzden "additive" (toplamsal) attention denir.

        score_t = v^T · tanh(W · h_t)
        weight_t = softmax(score_t)   (tüm zaman adımları üzerinde)
        context = Σ_t weight_t · h_t
    """
...
sample_reviews = [
    ...
    "The movie started great but the ending was disappointing and felt rushed",
]
