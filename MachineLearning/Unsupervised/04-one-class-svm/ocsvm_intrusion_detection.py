"""
AG GUVENLIGI - INTRUZYON (SALDIRI) TESPITI - One-Class SVM
Amac: Bir ag baglantisinin NORMAL mi yoksa SALDIRI/INTRUZYON mu oldugunu,
      SADECE NORMAL TRAFIK ORNEKLERINDEN OGRENEREK tespit etmek. One-Class
      SVM'in temel mantigi diger cogu algoritmadan farklidir: sadece TEK
      BIR SINIFIN (normal trafik) sinirini ogrenir, bu sinirin disina cikan
      HER SEYI anomali olarak isaretler. Bu, siber guvenlikte cok degerlidir
      cunku SALDIRI TURLERI SUREKLI DEGISIR - saldiri ornekleriyle egitilen
      bir model yeni bir saldiri turunu tanimayabilir, ama "normalin disinda"
      mantigiyla calisan bir model yeni saldiri turlerini de yakalayabilir.

      Bu proje ayni zamanda BigData/big-data-log-analytics projesindeki web
      log analitigiyle tematik olarak baglantilidir - ikisi de sunucu/ag
      trafigi verisiyle ilgilenir, ama bu proje "anomali tespiti" katmanini
      ekler.

Not: Gercek bir ag trafigi veri seti (orn. NSL-KDD, CICIDS) bu ortamda
     bulunmadigi icin, gercekci ag baglanti oruntularini yansitan SENTETIK
     bir veri seti uretilir. Saldiri etiketleri SADECE DEGERLENDIRME icin
     kullanilir, model egitiminde hic gorulmez.
"""
import os
import numpy as np
import pandas as pd
...
print("Ag Guvenligi Intruzyon Tespiti (One-Class SVM) - Veri Uretimi")
...
unique_ports_accessed = np.where(
    is_attack_true == 1,
    np.random.poisson(5, n),
...
df = pd.DataFrame({
    ...
    "unique_ports_accessed": unique_ports_accessed,
    "is_attack_true": is_attack_true
})
print(f"Uretilen baglanti sayisi: {len(df)}")
print(f"Gercek saldiri orani (SADECE degerlendirme icin): %{100*df['is_attack_true'].mean():.2f}")

feature_cols = ["duration_sec", "packet_size_avg", "num_failed_logins",
                 "connection_count_per_min", "bytes_sent", "unique_ports_accessed"]
...
print(f"\nEgitim SADECE normal trafikle yapiliyor: {normal_data.shape[0]} baglanti")
print("(One-Class SVM boylece 'normalin siniri' ogrenir, saldiri hic gormez)")
...
plt.title("One-Class SVM - İntrüzyon Tespiti Sonucu")
plt.tight_layout()
...
plt.xlabel("Anomali Skoru (yüksek = daha şüpheli)")
plt.title("Anomali Skoru Dağılımı (Gerçek Saldırı vs Normal)")
plt.legend(labels=["Saldırı", "Normal"])
plt.tight_layout()
...
plt.title("One-Class SVM İntrüzyon Tespiti (PCA ile 2 Boyut)")
plt.colorbar(scatter, label="0=Normal, 1=Saldırı")
plt.tight_layout()
